...
if [ ! -f "$BUILD_DIR/benchmarks/kmc_to_binary" ]; then
    echo "Error: kmc_to_binary not found at $BUILD_DIR/benchmarks/kmc_to_binary"
    echo "Please run: meson setup build && ninja -C build"
    exit 1
fi

KMC="$BUILD_DIR/subprojects/kmc/kmc"
KMC_TO_BINARY="$BUILD_DIR/benchmarks/kmc_to_binary"
